...
describe("ColourPicker", () => {
  it("renders the label", async () => {
    ...
    await renderWithProviders(<ColourPicker selectedColors={[]} setSelectedColors={mockSetColors} />);
  ...
  it("renders color circles for each selected color", async () => {
    ...
    await renderWithProviders(<ColourPicker selectedColors={colors} setSelectedColors={mockSetColors} />);
  ...
  it("hides add button when 5 colors selected", async () => {
    const mockSetColors = vi.fn();
    const colors = [{ hex: "#ff0000" }, { hex: "#00ff00" }, { hex: "#0000ff" }, { hex: "#ffff00" }, { hex: "#ff00ff" }];
    await renderWithProviders(<ColourPicker selectedColors={colors} setSelectedColors={mockSetColors} />);
