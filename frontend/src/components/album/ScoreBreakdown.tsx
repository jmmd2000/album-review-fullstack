...
const ScoreBreakdown = ({ baseScore, bonuses, finalScore, isOpen, onClose }: ScoreBreakdownProps) => (
  <Dialog isOpen={isOpen} onClose={onClose} title="Score Breakdown">
    <div className="bg-transparent backdrop-blur-xl p-4 rounded-md">
      ...
      {/* BONUSES */}
      {bonuses && (bonuses.qualityBonus > 0 || bonuses.perfectBonus > 0 || bonuses.consistencyBonus > 0 || bonuses.noWeakBonus > 0) && (
        <div className="text-sm font-medium text-green-500 mb-2">BONUSES</div>
      )}
      {bonuses && bonuses.qualityBonus > 0 && (
        <BreakdownRow title="Quality Tracks" description={getBonusDescription("qualityBonus")} bonus={formatBonus(bonuses.qualityBonus)} isPositive animationDelay={0.2} />
      )}
      {bonuses && bonuses.perfectBonus > 0 && (
        <BreakdownRow title="Perfect Tracks" description={getBonusDescription("perfectBonus")} bonus={formatBonus(bonuses.perfectBonus)} isPositive animationDelay={0.25} />
      )}
      {bonuses && bonuses.consistencyBonus > 0 && (
        <BreakdownRow title="Consistency" description={getBonusDescription("consistencyBonus")} bonus={formatBonus(bonuses.consistencyBonus)} isPositive animationDelay={0.3} />
      )}
      {bonuses && bonuses.noWeakBonus > 0 && (
        <BreakdownRow title="No Weak Tracks" description={getBonusDescription("noWeakBonus")} bonus={formatBonus(bonuses.noWeakBonus)} isPositive animationDelay={0.35} />
      )}
      ...
      {bonuses && bonuses.terriblePenalty < 0 && (
        <BreakdownRow title="Terrible Tracks" description={getBonusDescription("terriblePenalty")} bonus={formatBonus(bonuses.terriblePenalty)} isPositive={false} animationDelay={0.4} />
      )}
      {bonuses && bonuses.poorQualityPenalty < 0 && (
        <BreakdownRow title="Poor Quality" description={getBonusDescription("poorQualityPenalty")} bonus={formatBonus(bonuses.poorQualityPenalty)} isPositive={false} animationDelay={0.45} />
      )}
      {bonuses && bonuses.noStrongPenalty < 0 && (
        <BreakdownRow title="No Strong Tracks" description={getBonusDescription("noStrongPenalty")} bonus={formatBonus(bonuses.noStrongPenalty)} isPositive={false} animationDelay={0.5} />
      )}
      ...
      <div className="grid grid-cols-2 gap-x-6 gap-y-2 mb-4">
        ...
        <div className={`font-medium text-right ${bonuses && bonuses.totalBonus > 0 ? "text-green-600" : bonuses && bonuses.totalBonus < 0 ? "text-red-500" : "text-gray-400"}`}>
          {bonuses ? formatBonus(bonuses.totalBonus) : "0"}
        </div>
