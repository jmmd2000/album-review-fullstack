...
export const searchAlbums = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!query.query || query.query.trim() === "") throw new AppError("Search query is required.", 400);
...
export const getAlbum = asyncHandler(async (req: Request, res: Response) => {
  ...
  const spotifyAlbumData = await SpotifyService.getAlbum(albumID, req.query.includeGenres !== "false");
