...
async function setupData() {
  const g1 = await GenreModel.createGenre({ name: "Genre One", slug: "genre1" });
  const g2 = await GenreModel.createGenre({ name: "Genre Two", slug: "genre2" });
  const g3 = await GenreModel.createGenre({ name: "Genre Three", slug: "genre3" });
...
test("rating distribution groups albums by tier", async () => {
  const dist = await StatsService.getRatingDistribution();
